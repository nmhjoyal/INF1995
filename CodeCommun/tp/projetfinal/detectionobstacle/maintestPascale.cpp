...
uint8_t trouverObstacle(Robot& frobie){
	for (uint8_t i =0 ; i<= 3; i++){ //recolte 3 donnée pour savoir si c'est un mur ou un poteau
		obstacle[i] = frobie.detectionObstacle();
		_delay_ms(10/100); //delais entre chaque detection A DETERMINER
	}
				
	if ((obstacle[0] - obstacle[1]) <= 5 && (obstacle[0] - obstacle[2]) > 8)
		return poteau;
	else if ((obstacle[0] - obstacle[1]) <= 5 && (obstacle[0] - obstacle[2]) <= 5)
		return mur;
	else 
		return rien;
}
